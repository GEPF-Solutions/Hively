...
- Styling: Tailwind CSS v4 (`@tailwindcss/vite`). Dark theme, OKLCH palette, IBM Plex Mono for topic paths/code/patterns/ids and now the HIVELY wordmark too (colored to match the logo's yellow, not the brand-blue interactive accent — a second visual-identity pass moved the wordmark off Space Grotesk and back onto the monospace spec Design/README.md originally called for) — see `Design/README.md` → Design Tokens for the rest, including the chamfered-corner shape treatment on cards/panels/modals/buttons.
