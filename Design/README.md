...
- Saving a rule (creating or editing it) immediately re-applies it to every topic already matching its pattern, tracked or not — so adding a schema to a rule after the fact, say, pushes onto topics it already configured with no extra step. Rule application otherwise only ever fires once, at the moment a topic is first tracked, so without this a rule edit would never reach topics it had already configured.
- The Manage Rules panel also supports **"Apply to N now"** — the same bulk-apply, triggered manually, for topics that started matching a rule's pattern later without the rule itself being re-saved.
