...
**These reflect a second visual-identity pass, moving off the original gold/amber-dominant palette toward an enterprise-appropriate steel/slate blue** (the earlier all-gold direction read as generic and, once the logo's own yellow was reused for every button/nav/focus state too, risked colliding with the amber "untracked" status color — see git history around `hively.client/src/index.css` and the `Button`/`Modal`/`Input`/`Select`/`NavLink` components for the actual diff). Source of truth is always `index.css`'s `@theme` block plus those components; this section is a summary, not the authority.

**Logo/brand**: a small honeycomb (7-hexagon flower: 1 center + 6 ring cells) rendered as flat-color SVG polygons — no gradients. Center cell brightest (`oklch(0.9 0.15 98)`), 3 ring cells mid-gold (`oklch(0.83 0.17 95)`), 3 ring cells deeper amber (`oklch(0.68 0.14 85)`), thin dark-amber cell borders (`oklch(0.32 0.05 90)`). Sits next to the wordmark "HIVELY" (IBM Plex Mono, bold, wide tracking, colored to match the logo's mid-ring yellow via `--color-logo-yellow`) in the header — the wordmark used to run in Space Grotesk, but that's since been dropped in favor of the same mono face used for topic paths/code, so the app runs on two type families instead of three. See `hively.client/src/components/layout/HiveLogo.tsx` for the exact hexagon coordinates (a 7-cell honeycomb inscribed in a 100×100 viewBox) — recreate at any size/format from there.
...
- **Brand/interactive accent** (buttons, active nav/toggle underline, focus state, badges tied to brand): a steel/slate blue, `oklch(0.68 0.11 240)` (`--color-brand`, ink `--color-brand-ink` for text on a solid fill) — deliberately **not** the logo's yellow. The logomark and wordmark keep their own yellow (`--color-logo-yellow`, `oklch(0.83 0.17 95)`, matching the logo's mid-ring hex) as a fixed brand-mark color, but that yellow is never spread to buttons/nav/focus — keeping interactive chrome on one hue distinct from both the logo and the amber status color.
- **Status colors**, unchanged and kept semantically pure (never reused for brand/interactive chrome): compliant/cyan `oklch(0.75 0.13 200)`, warning/untracked/amber `oklch(0.78 0.15 80)`, danger/violations/red `oklch(0.7 0.18 25)`.
- Tag hues are arbitrary OKLCH hue angles chosen per-tag (`null` = neutral gray), fully independent of the brand accent — see `tagColor()` in `hively.client/src/utils/tagColor.ts`. An admin's tag color choice is never overridden by whatever the brand/interactive accent happens to be.

**Typography**:
- UI text: system font stack (`-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif`)
- Monospace (topic paths, code, patterns, IDs, **and now the header/login wordmark**): `IBM Plex Mono` (weights 400/500/600), loaded from Google Fonts — the whole app runs on this one mono face plus the system sans, no third display face.

**Shape**: two structural signatures instead of one. Status badges and tags stay unfilled outlined chips — transparent background + neutral (or per-tag) border + colored text, never a translucent tinted fill, at a modest ~6px radius, not full pills, not chamfered — chips/pills/small dots are deliberately exempt from the treatment below. Everything card/panel/modal/button-shaped instead gets a **chamfer**: two opposite corners (top-left, bottom-right) cut on a diagonal via `clip-path`, echoing a honeycomb cell's edge, in place of `border-radius` — see the `facet-sm`/`facet-md`/`facet-lg` (+ `facet-border`/`facet-border-strong`/`facet-accent`/etc. for the inset-box-shadow "border" that actually follows the cut edge, since a real CSS `border` doesn't) utilities in `index.css`, and the bundled `hv-card` utility for the common panel-shell case. Buttons collapse to exactly two types app-wide: `primary` (solid brand-blue fill, subtle tactile shadow) and `secondary` (quiet outline) — no separate "ghost" or "danger" look; a destructive action is a secondary button with red text, not a third variant. Active nav/view-toggle state is an underline, not a filled pill.
